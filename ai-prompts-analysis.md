# Toonflow-app AI 提示词分析文档

> 本文档整理了 Toonflow-app 项目中所有 AI 提示词，用于完善 `comic-drama-writer` 技能。
> 
> 生成日期：2026-02-24
...
| 序号 | 代码 | 名称 | 类型 | 字符数 |
|------|------|------|------|--------|
| 1 | `outlineScript-main` | 大纲故事线Agent | mainAgent | 9777 |
| 2 | `outlineScript-a1` | 大纲故事线Agent-故事师 | subAgent | 3879 |
| 3 | `outlineScript-a2` | 大纲故事线Agent-大纲师 | subAgent | 9026 |
| 4 | `outlineScript-director` | 大纲故事线Agent-导演 | subAgent | 8191 |
| 5 | `storyboard-main` | 分镜Agent | mainAgent | 3060 |
| 6 | `storyboard-segment` | 分镜Agent-片段分析师 | subAgent | 5429 |
| 7 | `storyboard-shot` | 分镜Agent-分镜师 | subAgent | 3396 |
| 8 | `generateImagePrompts` | 分镜Agent生图润色提示词 | system | 8480 |
| 9 | `role-polish` | 资产-角色提示词润色 | system | 2877 |
| 10 | `role-generateImage` | 资产-角色图片生成 | system | 4321 |
| 11 | `scene-polish` | 资产-场景提示词润色 | system | 3161 |
| 12 | `scene-generateImage` | 资产-场景图片生成 | system | 1502 |
| 13 | `storyboard-polish` | 资产-分镜提示词润色 | system | 1134 |
| 14 | `storyboard-generateImage` | 资产-分镜图片生成 | system | 5397 |
| 15 | `tool-polish` | 资产-道具提示词润色 | system | 3696 |
| 16 | `tool-generateImage` | 资产-道具图片生成 | system | 9914 |
| 17 | `script` | 剧本生成 | system | 8211 |
| 18 | `video-startEnd` | 视频提示词-首尾帧 | system | 182 |
| 19 | `video-multi` | 视频提示词-多图模式 | system | 170 |
| 20 | `video-single` | 视频提示词-单图模式 | system | 171 |
| 21 | `video-main` | 视频提示词-总规则 | system | 1626 |
| 22 | `video-text` | 视频提示词-文本模式 | system | 1250 |
...
| 类别 | 提示词代码 | 说明 |
|------|-----------|------|
| **大纲故事线** | `outlineScript-main` | 主协调 Agent |
| | `outlineScript-a1` | 故事师 - 负责故事线生成 |
| | `outlineScript-a2` | 大纲师 - 负责大纲生成 |
| | `outlineScript-director` | 导演 - 负责内容审核 |
| **分镜系统** | `storyboard-main` | 分镜主 Agent |
| | `storyboard-segment` | 片段分析师 |
| | `storyboard-shot` | 分镜师 |
| | `generateImagePrompts` | 分镜生图润色 |
| **资产生成** | `role-polish` | 角色润色 |
| | `role-generateImage` | 角色生图 |
| | `scene-polish` | 场景润色 |
| | `scene-generateImage` | 场景生图 |
| | `tool-polish` | 道具润色 |
| | `tool-generateImage` | 道具生图 |
| | `storyboard-polish` | 分镜润色 |
| | `storyboard-generateImage` | 分镜生图 |
| **剧本生成** | `script` | 剧本生成 |
| **视频生成** | `video-main` | 视频总规则 |
| | `video-startEnd` | 首尾帧模式 |
| | `video-multi` | 宫格模式 |
| | `video-single` | 单图模式 |
| | `video-text` | 文本模式 |
...
#### `outlineScript-a1` - 大纲故事线Agent-故事师

**类型**: `subAgent`  
**父级代码**: `"outlineScript-main"`
...
#### `outlineScript-a2` - 大纲故事线Agent-大纲师
...
#### `outlineScript-director` - 大纲故事线Agent-导演
...
#### `storyboard-segment` - 分镜Agent-片段分析师

**类型**: `subAgent`  
**父级代码**: `"storyboard-main"`
...
#### `generateImagePrompts` - 分镜Agent生图润色提示词
...
#### `storyboard-generateImage` - 资产-分镜图片生成
...
**文件位置**: `src/routes/storyboard/generateVideoPrompt.ts`

**功能**: 将静态分镜转化为 Motion Prompt（视频生成动作提示）
...
### 2. 剧本生成 User Prompt 模板

**文件位置**: `src/utils/generateScript.ts`

**功能**: 根据结构化大纲生成剧本

<details>
<summary>📄 点击展开完整 User Prompt 模板</summary>

```typescript
// formatEpisodePrompt 函数生成的结构化大纲格式
const episodePrompt = `
═══════════════════════════════════════
...
### 3. 资产润色 User Prompt 模板

**文件位置**: `src/routes/assets/polishPrompt.ts`

**功能**: 生成角色/场景/道具/分镜的润色提示词
...
### 4. 视频生成 User Prompt 模板

**文件位置**: `src/routes/video/generatePrompt.ts`
...
```typescript
const userPrompt = `Mode: ${getModeDescription(mode)}
...
interface t_prompts {
  ...
  code: string;                  // 提示词唯一代码
  name: string;                  // 提示词名称
  type: string;                  // 类型: mainAgent, subAgent, system
  parentCode: string | null;     // 父级代码
  defaultValue: string;          // 默认提示词内容
  customValue: string | null;    // 用户自定义内容（覆盖默认值）
}
```
...
## 针对 comic-drama-writer 技能的核心提示词

以下是漫画剧本创作的核心流程提示词，应优先整合到 `comic-drama-writer` 技能中：

### 核心工作流程

1. **故事线生成** → `outlineScript-a1` (故事师)
2. **大纲生成** → `outlineScript-a2` (大纲师)
3. **内容审核** → `outlineScript-director` (导演)
4. **剧本生成** → `script`
5. **分镜设计** → `storyboard-main` + `storyboard-shot` (分镜师)
6. **资产生成** → `role-*`, `scene-*`, `tool-*` 系列
7. **视频动态化** → `video-*` 系列 + Motion Prompt

### 建议优先提取的关键提示词

| 优先级 | 提示词代码 | 用途 |
|--------|-----------|------|
| P0 | `outlineScript-a1` | 故事线生成核心逻辑 |
| P0 | `outlineScript-a2` | 大纲生成核心逻辑 |
| P0 | `script` | 剧本生成规范 |
| P1 | `storyboard-shot` | 分镜设计规则 |
| P1 | `generateImagePrompts` | 分镜生图规范 |
| P2 | `role-polish` | 角色描述润色 |
| P2 | `scene-polish` | 场景描述润色 |
...
项目使用数据库存储提示词，支持自定义覆盖：

```typescript
// 获取提示词（优先使用 customValue）
const prompts = await u.db("t_prompts").where("code", code).first();
const promptContent = prompts?.customValue || prompts?.defaultValue;
```
...
```typescript
// 获取 AI 配置
const apiConfig = await u.getPromptAi("promptType");
...
大纲系统采用多 Agent 协作模式：
- **mainAgent** (`outlineScript-main`): 协调调度
- **subAgent** (`outlineScript-a1`, `outlineScript-a2`, `outlineScript-director`): 执行具体任务
